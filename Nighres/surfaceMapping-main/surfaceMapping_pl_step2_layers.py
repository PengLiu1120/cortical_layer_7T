...
# -------------------
# Define BIDS path
BIDS_path = '/surfacemapping/frj712/'
...
map_data = BIDS_path + 'derivatives/sub-frj/func/sub-frj_task-pRF_bold_mean.nii.gz'
...
transform_data = BIDS_path + 'derivatives/sub-frj/pRF/'
	...
	if os.path.isdir(transform_data):
		...
		if isinstance(transform_data, list):
			for tmp in transform_data:
				...
				# This will remove the sub-<label> string if the data is already BIDS
				# formatted. It'll be added during creation of the output data later
				# again./Users/pliu/Documents/Programmes/Nighres/surfaceMapping-main/surfaceMapping_pl_layers.py
				if 'sub-' + sub in tmp:
					tmp = tmp[8:]
...
# ---------------------
# Here, we extract the layers 3 to 10, to cover the "deep" layers. 
# Afterwards the information is mapped on the original and inflated
# surface of the middle layer.
print('')
print('*****************************************************')
...
###########################################################################
# 12.7. Extract middle cortical layers and map on surface (left hemisphere)
# ---------------------
# Here, we extract the layers 11 to 17, to cover the "middle" layers. 
# Afterwards the information is mapped on the original and inflated
# surface of the middle layer.
print('')
print('*****************************************************')
...
if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + filename + '_leftHemisphere_extractedLayers-middleLayers_mean.nii.gz')) and reprocess != True:
	print('File exists already. Skipping process.')
else:
	tmp = profile.get_fdata()[:,:,:,11:17]
	extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
	nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + filename + '_leftHemisphere_extractedLayers-middleLayers.nii.gz'))
	meanProfile = mean_img(extractedLayers)
	nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + filename + '_leftHemisphere_extractedLayers-middleLayers_mean.nii.gz'))
	del tmp
...
nighres.surface.surface_mesh_mapping(
                        ...
                        overwrite=reprocess,
                        output_dir=out_dir,
                        file_name='sub-' + sub + filename + '_leftHemisphere_extractedLayers-middleLayers.vtk')

###########################################################################
# 12.9. Extract superficial cortical layers and map on surface (left hemisphere)
# ---------------------
# Here, we extract the layers 18 to 21, to cover the "superficial" layers. 
# Afterwards the information is mapped on the original and inflated
# surface of the middle layer.
print('')
print('*****************************************************')
...
###########################################################################
# 12.10. Profile sampling of additional data of deep layers and mapping it onto
# the surface (left hemisphere)
# ---------------------
# Here, we sample the information of the additional data onto all layers.
# And we extract the layers 3 to 10, removing layers close to CSF to avoid
...
if map_file_onto_surface:
	...
	print('')
	print('*****************************************************')
	print('* Extracting all layers of additional data (left hemisphere)')
	print('* Started at: ' + strftime("%Y-%m-%d %H:%M:%S", localtime()))
	print('*****************************************************')
	...
	print('')
	print('*****************************************************')
	print('* Extracting deep layers of additional data (left hemisphere)')
	print('* Started at: ' + strftime("%Y-%m-%d %H:%M:%S", localtime()))
	print('*****************************************************')

	if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_leftHemisphere_extractedLayers-deepLayers_mean.nii.gz')) and reprocess != True:
		print('File exists already. Skipping process.')
		meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_leftHemisphere_extractedLayers-deepLayers_mean.nii.gz'))
	else:
		tmp = profile.get_fdata()[:,:,:,3:10]
		extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
		nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_leftHemisphere_extractedLayers-deepLayers.nii.gz'))
		meanProfile = mean_img(extractedLayers)
		nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_leftHemisphere_extractedLayers-deepLayers_mean.nii.gz'))
		del tmp
	...
	nighres.surface.surface_mesh_mapping(
		                ...
		                overwrite=reprocess,
		                output_dir=out_dir,
		                file_name='sub-' + sub + '_' + map_data_output + '_leftHemisphere_extractedLayers-deepLayers.vtk')

	print('*****************************************************')
	print('* Extracting middle layers of additional data (left hemisphere)')
	print('* Started at: ' + strftime("%Y-%m-%d %H:%M:%S", localtime()))
	print('*****************************************************')

	if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_leftHemisphere_extractedLayers-middleLayers_mean.nii.gz')) and reprocess != True:
		print('File exists already. Skipping process.')
		meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_leftHemisphere_extractedLayers-middleLayers_mean.nii.gz'))
	else:
		tmp = profile.get_fdata()[:,:,:,11:17]
		extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
		nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_leftHemisphere_extractedLayers-middleLayers.nii.gz'))
		meanProfile = mean_img(extractedLayers)
		nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_leftHemisphere_extractedLayers-middleLayers_mean.nii.gz'))
		del tmp
	...
	nighres.surface.surface_mesh_mapping(
		                ...
		                overwrite=reprocess,
		                output_dir=out_dir,
		                file_name='sub-' + sub + '_' + map_data_output + '_leftHemisphere_extractedLayers-middleLayers.vtk')

	print('*****************************************************')
	print('* Extracting superficial layers of additional data (left hemisphere)')
	print('* Started at: ' + strftime("%Y-%m-%d %H:%M:%S", localtime()))
	print('*****************************************************')

	if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_leftHemisphere_extractedLayers-superficialLayers_mean.nii.gz')) and reprocess != True:
		print('File exists already. Skipping process.')
		meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_leftHemisphere_extractedLayers-superficialLayers_mean.nii.gz'))
	else:
		tmp = profile.get_fdata()[:,:,:,18:21]
		extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
		nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_leftHemisphere_extractedLayers-superficialLayers.nii.gz'))
		meanProfile = mean_img(extractedLayers)
		nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_leftHemisphere_extractedLayers-superficialLayers_mean.nii.gz'))
		del tmp
	...
	nighres.surface.surface_mesh_mapping(
		                ...
		                overwrite=reprocess,
		                output_dir=out_dir,
		                file_name='sub-' + sub + '_' + map_data_output + '_leftHemisphere_extractedLayers-superficialLayers.vtk')
...
if map_transform_file_onto_surface:
	...
	if isinstance(transform_data_output,list):
		...
		for i in range(length):
			...
			print('')
			print('*****************************************************')
			print('* Profile sampling and extracting deep cortical layers of additional data (left hemisphere)')
			print('*****************************************************')
			...
			if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_leftHemisphere_extractedLayers-deepLayers_mean.nii.gz')) and reprocess != True:
				print('File exists already. Skipping process.')
				meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_leftHemisphere_extractedLayers-deepLayers_mean.nii.gz'))
			else:
				tmp = profile.get_fdata()[:,:,:,3:10]
				extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
				nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_leftHemisphere_extractedLayers-deepLayers.nii.gz'))
				meanProfile = mean_img(extractedLayers)
				nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_leftHemisphere_extractedLayers-deepLayers_mean.nii.gz'))
				del tmp
			...
			nighres.surface.surface_mesh_mapping(
						...
						overwrite=reprocess,
						output_dir=out_dir,
						file_name='sub-' + sub + '_' + transform_data_output[i] + '_leftHemisphere_extractedLayers-deepLayers.vtk')
            
			print('')
			print('*****************************************************')
			print('* Profile sampling and extracting middle cortical layers of additional data (left hemisphere)')
			print('*****************************************************')
			...
			if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_leftHemisphere_extractedLayers-middleLayers_mean.nii.gz')) and reprocess != True:
				print('File exists already. Skipping process.')
				meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_leftHemisphere_extractedLayers-middleLayers_mean.nii.gz'))
			else:
				tmp = profile.get_fdata()[:,:,:,11:17]
				extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
				nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_leftHemisphere_extractedLayers-middleLayers.nii.gz'))
				meanProfile = mean_img(extractedLayers)
				nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_leftHemisphere_extractedLayers-middleLayers_mean.nii.gz'))
				del tmp
			...
			nighres.surface.surface_mesh_mapping(
						...
						overwrite=reprocess,
						output_dir=out_dir,
						file_name='sub-' + sub + '_' + transform_data_output[i] + '_leftHemisphere_extractedLayers-middleLayers.vtk')
            
			print('')
			print('*****************************************************')
			print('* Profile sampling and extracting superficial cortical layers of additional data (left hemisphere)')
			print('*****************************************************')
			...
			if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_leftHemisphere_extractedLayers-superficialLayers_mean.nii.gz')) and reprocess != True:
				print('File exists already. Skipping process.')
				meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_leftHemisphere_extractedLayers-superficialLayers_mean.nii.gz'))
			else:
				tmp = profile.get_fdata()[:,:,:,18:21]
				extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
				nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_leftHemisphere_extractedLayers-superficialLayers.nii.gz'))
				meanProfile = mean_img(extractedLayers)
				nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_leftHemisphere_extractedLayers-superficialLayers_mean.nii.gz'))
				del tmp
			...
			nighres.surface.surface_mesh_mapping(
						...
						overwrite=reprocess,
						output_dir=out_dir,
						file_name='sub-' + sub + '_' + transform_data_output[i] + '_leftHemisphere_extractedLayers-superficialLayers.vtk')
	else:
		profile = nighres.laminar.profile_sampling(
				        profile_surface_image=layers,
		...
		print('')
		print('*****************************************************')
		print('* Extracting deep cortical layers of additional data (left hemisphere)')
		print('* Started at: ' + strftime("%Y-%m-%d %H:%M:%S", localtime()))
		print('*****************************************************')

		if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_leftHemisphere_extractedLayers-deepLayers_mean.nii.gz')) and reprocess != True:
			print('File exists already. Skipping process.')
			meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_leftHemisphere_extractedLayers-deepLayers_mean.nii.gz'))
		else:
			tmp = profile.get_fdata()[:,:,:,3:10]
			extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
			nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_leftHemisphere_extractedLayers-deepLayers.nii.gz'))
			meanProfile = mean_img(extractedLayers)
			nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_leftHemisphere_extractedLayers-deepLayers_mean.nii.gz'))
			del tmp
		...
		nighres.surface.surface_mesh_mapping(
				        ...
				        overwrite=reprocess,
				        output_dir=out_dir,
				        file_name='sub-' + sub + '_' + transform_data_output + '_leftHemisphere_extractedLayers-deepLayers.vtk')
        
		print('')
		print('*****************************************************')
		print('* Extracting middle cortical layers of additional data (left hemisphere)')
		print('* Started at: ' + strftime("%Y-%m-%d %H:%M:%S", localtime()))
		print('*****************************************************')

		if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_leftHemisphere_extractedLayers-middleLayers_mean.nii.gz')) and reprocess != True:
			print('File exists already. Skipping process.')
			meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_leftHemisphere_extractedLayers-middleLayers_mean.nii.gz'))
		else:
			tmp = profile.get_fdata()[:,:,:,11:17]
			extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
			nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_leftHemisphere_extractedLayers-middleLayers.nii.gz'))
			meanProfile = mean_img(extractedLayers)
			nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_leftHemisphere_extractedLayers-middleLayers_mean.nii.gz'))
			del tmp
		...
		nighres.surface.surface_mesh_mapping(
				        ...
				        overwrite=reprocess,
				        output_dir=out_dir,
				        file_name='sub-' + sub + '_' + transform_data_output + '_leftHemisphere_extractedLayers-middleLayers.vtk')
        
		print('')
		print('*****************************************************')
		print('* Extracting superficial cortical layers of additional data (left hemisphere)')
		print('* Started at: ' + strftime("%Y-%m-%d %H:%M:%S", localtime()))
		print('*****************************************************')

		if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_leftHemisphere_extractedLayers-superficialLayers_mean.nii.gz')) and reprocess != True:
			print('File exists already. Skipping process.')
			meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_leftHemisphere_extractedLayers-superficialLayers_mean.nii.gz'))
		else:
			tmp = profile.get_fdata()[:,:,:,18:21]
			extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
			nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_leftHemisphere_extractedLayers-superficialLayers.nii.gz'))
			meanProfile = mean_img(extractedLayers)
			nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_leftHemisphere_extractedLayers-superficialLayers_mean.nii.gz'))
			del tmp
		...
		nighres.surface.surface_mesh_mapping(
				        ...
				        overwrite=reprocess,
				        output_dir=out_dir,
				        file_name='sub-' + sub + '_' + transform_data_output + '_leftHemisphere_extractedLayers-superficialLayers.vtk')
...
# ---------------------
# Here, we extract the layers 3 to 21, removing layers close to the white
# matter and cerebrospinal fluid to avoid partial volume effects.
# Afterwards the information is mapped on the original and inflated
# surface of the middle layer.
print('')
...
###########################################################################
# 16.7. Extract middle cortical layers and map on surface (right hemisphere)
# ---------------------
...
print('')
print('*****************************************************')
print('* Extracting middle cortical layers (right hemisphere)')
print('* Started at: ' + strftime("%Y-%m-%d %H:%M:%S", localtime()))
print('*****************************************************')

if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + filename + '_rightHemisphere_extractedLayers-middleLayers_mean.nii.gz')) and reprocess != True:
	print('File exists already. Skipping process.')
else:
	tmp = profile.get_fdata()[:,:,:,7:11]
	extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
	nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + filename + '_rightHemisphere_extractedLayers-middleLayers.nii.gz'))
	meanProfile = mean_img(extractedLayers)
	nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + filename + '_rightHemisphere_extractedLayers-middleLayers_mean.nii.gz'))
	del tmp
...
nighres.surface.surface_mesh_mapping(
                        ...
                        overwrite=reprocess,
                        output_dir=out_dir,
                        file_name='sub-' + sub + filename + '_rightHemisphere_extractedLayers-middleLayers.vtk')
...
if map_file_onto_surface:
	...
	print('')
	print('*****************************************************')
	print('* Extracting deep cortical layers of additional data (right hemisphere)')
	print('* Started at: ' + strftime("%Y-%m-%d %H:%M:%S", localtime()))
	print('*****************************************************')

	if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_rightHemisphere_extractedLayers-deepLayers_mean.nii.gz')) and reprocess != True:
		print('File exists already. Skipping process.')
		meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_rightHemisphere_extractedLayers-deepLayers_mean.nii.gz'))
	else:
		tmp = profile.get_fdata()[:,:,:,3:10]
		extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
		nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_rightHemisphere_extractedLayers-deepLayers.nii.gz'))
		meanProfile = mean_img(extractedLayers)
		nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_rightHemisphere_extractedLayers-deepLayers_mean.nii.gz'))
		del tmp
	...
	nighres.surface.surface_mesh_mapping(
		                ...
		                overwrite=reprocess,
		                output_dir=out_dir,
		                file_name='sub-' + sub + '_' + map_data_output + '_rightHemisphere_extractedLayers-deepLayers.vtk')
    
	print('')
	print('*****************************************************')
	print('* Extracting middle cortical layers of additional data (right hemisphere)')
	print('* Started at: ' + strftime("%Y-%m-%d %H:%M:%S", localtime()))
	print('*****************************************************')

	if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_rightHemisphere_extractedLayers-middleLayers_mean.nii.gz')) and reprocess != True:
		print('File exists already. Skipping process.')
		meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_rightHemisphere_extractedLayers-middleLayers_mean.nii.gz'))
	else:
		tmp = profile.get_fdata()[:,:,:,11:17]
		extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
		nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_rightHemisphere_extractedLayers-middleLayers.nii.gz'))
		meanProfile = mean_img(extractedLayers)
		nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_rightHemisphere_extractedLayers-middleLayers_mean.nii.gz'))
		del tmp
	...
	nighres.surface.surface_mesh_mapping(
		                ...
		                overwrite=reprocess,
		                output_dir=out_dir,
		                file_name='sub-' + sub + '_' + map_data_output + '_rightHemisphere_extractedLayers-middleLayers.vtk')
    
	print('')
	print('*****************************************************')
	print('* Extracting superficial cortical layers of additional data (right hemisphere)')
	print('* Started at: ' + strftime("%Y-%m-%d %H:%M:%S", localtime()))
	print('*****************************************************')

	if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_rightHemisphere_extractedLayers-superficialLayers_mean.nii.gz')) and reprocess != True:
		print('File exists already. Skipping process.')
		meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_rightHemisphere_extractedLayers-superficialLayers_mean.nii.gz'))
	else:
		tmp = profile.get_fdata()[:,:,:,18:21]
		extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
		nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_rightHemisphere_extractedLayers-superficialLayers.nii.gz'))
		meanProfile = mean_img(extractedLayers)
		nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_' + map_data_output + '_rightHemisphere_extractedLayers-superficialLayers_mean.nii.gz'))
		del tmp
	...
	nighres.surface.surface_mesh_mapping(
		                ...
		                overwrite=reprocess,
		                output_dir=out_dir,
		                file_name='sub-' + sub + '_' + map_data_output + '_rightHemisphere_extractedLayers-superficialLayers.vtk')
...
if map_transform_file_onto_surface:
	...
	if isinstance(transform_data_output,list):
		...
		for i in range(length):
			...
			print('')
			print('*****************************************************')
			print('* Profile sampling and extracting deep cortical layers of additional data (right hemisphere)')
			print('*****************************************************')
			profile = nighres.laminar.profile_sampling(
						profile_surface_image=layers,
						intensity_image=transform_data_rightHemisphere[i],
			...
			if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_rightHemisphere_extractedLayers-deepLayers_mean.nii.gz')) and reprocess != True:
				print('File exists already. Skipping process.')
				meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_rightHemisphere_extractedLayers-deepLayers_mean.nii.gz'))
			else:
				tmp = profile.get_fdata()[:,:,:,3:10]
				extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
				nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_rightHemisphere_extractedLayers-deepLayers.nii.gz'))
				meanProfile = mean_img(extractedLayers)
				nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_rightHemisphere_extractedLayers-deepLayers_mean.nii.gz'))
				del tmp
			...
			nighres.surface.surface_mesh_mapping(
						...
						overwrite=reprocess,
						output_dir=out_dir,
						file_name='sub-' + sub + '_' + transform_data_output[i] + '_rightHemisphere_extractedLayers-deepLayers.vtk')
            
			print('')
			print('*****************************************************')
			print('* Profile sampling and extracting middle cortical layers of additional data (right hemisphere)')
			print('*****************************************************')
			profile = nighres.laminar.profile_sampling(
						profile_surface_image=layers,
						intensity_image=transform_data_rightHemisphere[i],
			...
			if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_rightHemisphere_extractedLayers-middleLayers_mean.nii.gz')) and reprocess != True:
				print('File exists already. Skipping process.')
				meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_rightHemisphere_extractedLayers-middleLayers_mean.nii.gz'))
			else:
				tmp = profile.get_fdata()[:,:,:,11:17]
				extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
				nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_rightHemisphere_extractedLayers-middleLayers.nii.gz'))
				meanProfile = mean_img(extractedLayers)
				nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_rightHemisphere_extractedLayers-middleLayers_mean.nii.gz'))
				del tmp
			...
			nighres.surface.surface_mesh_mapping(
						...
						overwrite=reprocess,
						output_dir=out_dir,
						file_name='sub-' + sub + '_' + transform_data_output[i] + '_rightHemisphere_extractedLayers-middleLayers.vtk')
            
			print('')
			print('*****************************************************')
			print('* Profile sampling and extracting superficial cortical layers of additional data (right hemisphere)')
			print('*****************************************************')
			profile = nighres.laminar.profile_sampling(
						profile_surface_image=layers,
						intensity_image=transform_data_rightHemisphere[i],
			...
			if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_rightHemisphere_extractedLayers-superficialLayers_mean.nii.gz')) and reprocess != True:
				print('File exists already. Skipping process.')
				meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_rightHemisphere_extractedLayers-superficialLayers_mean.nii.gz'))
			else:
				tmp = profile.get_fdata()[:,:,:,18:21]
				extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
				nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_rightHemisphere_extractedLayers-superficialLayers.nii.gz'))
				meanProfile = mean_img(extractedLayers)
				nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output[i] + '_rightHemisphere_extractedLayers-superficialLayers_mean.nii.gz'))
				del tmp
			...
			nighres.surface.surface_mesh_mapping(
						...
						overwrite=reprocess,
						output_dir=out_dir,
						file_name='sub-' + sub + '_' + transform_data_output[i] + '_rightHemisphere_extractedLayers-superficialLayers.vtk')
	else:
		profile = nighres.laminar.profile_sampling(
				        profile_surface_image=layers,
		...
		if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_rightHemisphere_extractedLayers-deepLayers_mean.nii.gz')) and reprocess != True:
			print('File exists already. Skipping process.')
			meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_rightHemisphere_extractedLayers-deepLayers_mean.nii.gz'))
		else:
			tmp = profile.get_fdata()[:,:,:,3:10]
			extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
			nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_rightHemisphere_extractedLayers-deepLayers.nii.gz'))
			meanProfile = mean_img(extractedLayers)
			nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_rightHemisphere_extractedLayers-deepLayers_mean.nii.gz'))
			del tmp
		...
		nighres.surface.surface_mesh_mapping(
				        ...
				        overwrite=reprocess,
				        output_dir=out_dir,
				        file_name='sub-' + sub + '_' + transform_data_output + '_rightHemisphere_extractedLayers-deepLayers.vtk')
		...
		if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_rightHemisphere_extractedLayers-middleLayers_mean.nii.gz')) and reprocess != True:
			print('File exists already. Skipping process.')
			meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_rightHemisphere_extractedLayers-middleLayers_mean.nii.gz'))
		else:
			tmp = profile.get_fdata()[:,:,:,11:17]
			extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
			nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_rightHemisphere_extractedLayers-middleLayers.nii.gz'))
			meanProfile = mean_img(extractedLayers)
			nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_rightHemisphere_extractedLayers-middleLayers_mean.nii.gz'))
			del tmp
		...
		nighres.surface.surface_mesh_mapping(
				        ...
				        overwrite=reprocess,
				        output_dir=out_dir,
				        file_name='sub-' + sub + '_' + transform_data_output + '_rightHemisphere_extractedLayers-middleLayers.vtk')
		...
		if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_rightHemisphere_extractedLayers-superficialLayers_mean.nii.gz')) and reprocess != True:
			print('File exists already. Skipping process.')
			meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_rightHemisphere_extractedLayers-superficialLayers_mean.nii.gz'))
		else:
			tmp = profile.get_fdata()[:,:,:,18:21]
			extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
			nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_rightHemisphere_extractedLayers-superficialLayers.nii.gz'))
			meanProfile = mean_img(extractedLayers)
			nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_' + transform_data_output + '_rightHemisphere_extractedLayers-superficialLayers_mean.nii.gz'))
			del tmp
		...
		nighres.surface.surface_mesh_mapping(
				        ...
				        overwrite=reprocess,
				        output_dir=out_dir,
				        file_name='sub-' + sub + '_' + transform_data_output + '_rightHemisphere_extractedLayers-superficialLayers.vtk')

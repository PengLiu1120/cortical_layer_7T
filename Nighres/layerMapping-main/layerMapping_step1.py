...
BIDS_path = '/Volumes/IKND/AG_Kuehn/Peng/LayerPRF/SurfaceMapping/sst050/'
...
map_data = ''
#map_data = BIDS_path + 'derivatives/sub-lpr/func/sub-lpr_task-pRF_bold_mean.nii.gz'
...
if os.path.isfile(os.path.join(FreeSurfer_path, subID, 'scripts', 'recon-all-status.log')) and reprocess != True:
	...
	subprocess.run(["recon-all", "-all", "-notal-check", "-hires", "-parallel", "-sd", FreeSurfer_path, "-xmask", mask, "-i", T1w, "-s", subID])

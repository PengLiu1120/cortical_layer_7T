...
trs_dir = '/home/juli/projects/onehander/results/ReadOutLayerData_T1_HandFaceFoot/thrs__better_for_nom';
...
for ind = 1:length(subject)
    ...
    for h = 1:length(hemi)
        ...
        for r = 1:length(res)
             ...
             try
                load(sprintf('%s_T1_mface_21layer_mean_thrs_%s_%s_70_minROI.mat',subject{ind},hemi{h},res{r}));
                load(sprintf('%s_T1_mhand_21layer_mean_thrs_%s_%s_70_minROI.mat',subject{ind},hemi{h},res{r}));
                load(sprintf('%s_T1_mfoot_21layer_mean_thrs_%s_%s_70_minROI.mat',subject{ind},hemi{h},res{r}));
                actionmaps = 1;
             catch
    ...
    saveas(figure(ind),sprintf('%s_qT1_mFaceHandFoot_rotated_%s_70_minROI.eps',subject{ind},res{r}),'epsc');
    saveas(figure(ind),sprintf('%s_qT1_mFaceHandFoot_rotated_%s_70_minROI.fig',subject{ind},res{r}));
    saveas(figure(ind),sprintf('%s_qT1_mFaceHandFoot_rotated_%s_70_minROI.jpeg',subject{ind},res{r}));

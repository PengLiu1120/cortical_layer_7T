...
set(gca,'View',[90 -90])
xlim([1 21]);
ylim([-0.03 0]); % Resets y-axis dimensions
hold on;
...
for ind = 1:length(subject)
    ...
    for h = 1:length(hemi)
        ...
        for r = 1:length(res)
             ...
             try
                load(sprintf('%s_nQSM_mface_21layer_mean_thrs_%s_%s_70_minROI.mat',subject{ind},hemi{h},res{r}));
                load(sprintf('%s_nQSM_mhand_21layer_mean_thrs_%s_%s_70_minROI.mat',subject{ind},hemi{h},res{r}));
                load(sprintf('%s_nQSM_mfoot_21layer_mean_thrs_%s_%s_70_minROI.mat',subject{ind},hemi{h},res{r}));
                actionmaps = 1;
             catch
             ...
             if actionmaps == 1
                 ...
                 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                 table_allsubjects_fingers_long = [table_allsubjects_fingers_long; mean_T1_D1_21_neg;mean_T1_D2_21_neg;mean_T1_D3_21_neg];
                 ...
                 table_allsubjects_fingers_short(ind,:) = [transpose(mean_T1_D1_21_neg(:,1)) transpose(mean_T1_D2_21_neg(:,1)) transpose(mean_T1_D3_21_neg(:,1)) subject_code(ind) ind transpose(mean_T1_D1_21_neg(1,6)) transpose(mean_T1_D2_21_neg(1,6)) transpose(mean_T1_D3_21_neg(1,6)) h r];
                 T(ind,:) = [transpose(mean_T1_D1_21_neg(:,1)) transpose(mean_T1_D2_21_neg(:,1)) transpose(mean_T1_D3_21_neg(:,1)) subject_code(ind) ind transpose(mean_T1_D1_21_neg(1,6)) transpose(mean_T1_D2_21_neg(1,6)) transpose(mean_T1_D3_21_neg(1,6)) h r];
                     
                 % specify datasets that contain NaN values in QSM of layers #4 - #19:
                 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                 tmp = [mean_T1_D1_21_neg(:,1) mean_T1_D2_21_neg(:,1) mean_T1_D3_21_neg(:,1)];
    ...
    saveas(figure(ind),sprintf('%s_nQSM_mFaceHandFoot_rotated_%s_70_minROI.eps',subject{ind},res{r}),'epsc');
    saveas(figure(ind),sprintf('%s_nQSM_mFaceHandFoot_rotated_%s_70_minROI.fig',subject{ind},res{r}));
    saveas(figure(ind),sprintf('%s_nQSM_mFaceHandFoot_rotated_%s_70_minROI.jpeg',subject{ind},res{r}));
